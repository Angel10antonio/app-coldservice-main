...
const ConsultarEquiposConfiguradosScreen = () => {
  ...
  useEffect(() => {
    const obtenerEquipo = async () => {
      try {
        const snapshot = await db.collection('equipos_configurados').limit(1).get();
        if (!snapshot.empty) {
    ...
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        <View style={styles.equipoContainer}>
          ...
          <Text style={styles.info}>{new Date(equipo.fechaConfiguracion).toLocaleDateString()}</Text>
          ...
          <Text style={styles.info}>{new Date(equipo.fechaUltimoMantenimiento).toLocaleDateString()}</Text>
        </View>
      </ScrollView>
    </View>
  );
